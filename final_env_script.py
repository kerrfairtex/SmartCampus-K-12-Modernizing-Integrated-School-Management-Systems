...
def process_line(key, value, new_app_key):
    ...
    if needs_quoting(trimmed_value) and not is_quoted(trimmed_value):
        ...
        # We return the original key and value (without changing the spacing? but we want to remove extra spaces)
        # We'll return the key and the trimmed value (without extra spaces) because the .env parser doesn't care about spaces around the value.
        return f'{key}={trimmed_value}'
...
def main():
    ...
    with open(backup_path, 'r') as f:
        for line in f:
            ...
            processed = process_line(key, value, new_app_key)
